...
def run(argv):
    ...
    try:
        ...
        print(
            'Train_example_dataset.py -f <file name> -d <directory name> -k <samples from miniority> -r <ratio of new samples >')
        sys.exit(2)
    print(opts)
    for opt, arg in opts:
        if opt == '-h':
    ...
    global_count = sum(
        [tf.data.experimental.cardinality(clients_batched[client_name]).numpy() for client_name in client_names]) * bs
    ...
    for comm_round in range(comms_round):
        ...
        for client in client_names:
            ...
            # first calculate the total training data points across clinets
            global_count = sum(
                [tf.data.experimental.cardinality(clients_batched[client_name]).numpy() for client_name in
                 client_names]) * bs
